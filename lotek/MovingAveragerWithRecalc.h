...
/*
    ...
    A moving average with full recalculation at specified intervals
    to avoid catastrophic loss of precision.

    An alternative to KahanMovingAverager which hopefully is more
    amenable to vectorization.
    Licence: GPL version 2 or later
*/

#ifndef _MOVING_AVERAGER_WITH_RECALC_H
#define _MOVING_AVERAGER_WITH_RECALC_H

#include <boost/circular_buffer.hpp>

template < typename DATATYPE, typename AVGTYPE > 
class MovingAveragerWithRecalc {
 public:
  MovingAveragerWithRecalc (size_t n = 1) :
  buf(n),
  m_total(0)
    {
    };
...
#endif //  _MOVING_AVERAGER_WITH_RECALC_H
