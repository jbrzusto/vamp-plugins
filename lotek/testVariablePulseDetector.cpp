...
#include <stdlib.h>
#include <time.h>
#include "VariablePulseDetector.h"
...
main (int argc, char *argv[])
{
  if (argc < 6) {
    std::cout << 
"Usage: testVariablePulseDetector MINWIDTH MAXWIDTH MINDIFF MAXPROB NUMSAMPLES\n\
...
only be detected when it is the best edge for N samples in either direction.\n";
  ...
  VariablePulseDetector < float > pd (min_width, max_width, mindiff, maxprob);
