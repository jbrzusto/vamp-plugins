...
#ifndef _VARIABLE_PULSE_DETECTOR_H
#define _VARIABLE_PULSE_DETECTOR_H

#include "EdgeDetector.h"

// VariablePulseDetector: find pulses in a data stream.  A pulse is a rising
...
class VariablePulseDetector {
 public:

  typedef typename boost::circular_buffer < TYPE > ::const_array_range array_range_t;
  
  VariablePulseDetector(size_t min_width=0, size_t max_width=0, TYPE min_diff=0, double max_prob=0) :
    min_width(min_width),
    max_width(max_width),
    min_diff(min_diff),
    max_prob(max_prob),
...
#endif //  _VARIABLE_PULSE_DETECTOR_H
