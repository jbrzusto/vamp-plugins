...
#ifndef _STAT_PULSE_FINDER_H
#define _STAT_PULSE_FINDER_H

// StatPulsefinder: search an input sequence for pulses of width n, using this probe function:
//
//            1/n|              ---- (n) ----
//               |
//               |
...
// input sequence reaches a maximum over a window of 2n consecutive
...
// Keep track of variance of power within the given window to allow for
...
class PulseFinder {
 public:
  PulseFinder (int recalc_interval=131072, size_t n = 2)
    m_pulse_width(n),
    m_noise_width(n / 2),
    m_back(2*n)
    m_div_signal (n),
    m_div_noise (2 * m),
    m_mult_diff (2 * m / (DATATYPE) n),
    m_sample_buf (n + 2*m),
